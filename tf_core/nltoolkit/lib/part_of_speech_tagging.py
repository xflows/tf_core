...
class CustomPerceptronTagger(TaggerI):

    '''
    Greedy Averaged Perceptron tagger, as implemented by Matthew Honnibal with a fix that
    makes it not crash on 0 lenth tokens during training.
    See more implementation details here:
        http://spacy.io/blog/part-of-speech-POS-tagger-in-python/
...
def tnt_pos_tagger(input_dict):
    ...
    print(tnt_tagger.tagdata([['test', 'is'], ['hey', 'there']]))
...
def nlp4j_tagger(input_dict, widget):
    ...
    nlp4j_tagger = nlp4j.NLP4JTagger(settings.NLP4J_BINARIES, settings.NLP4J_CONFIG, NLP4J_INPUT, NLP4J_INPUT)
